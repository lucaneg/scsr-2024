...
public class Parity implements BaseNonRelationalValueDomain<Parity>{
	...
	public static final Parity BOTTOM = new Parity(0);
	...
	@Override
	public Parity lubAux(Parity o) throws SemanticException {
		return TOP;
	}

	@Override
	public boolean lessOrEqualAux(Parity o) throws SemanticException {
		return false;
	}
	...
	public Parity evalBinaryExpression(BinaryOperator operator, Parity left, Parity right, ProgramPoint pp, SemanticOracle oracle) {
		if (left.isTop()) 		return top();
		else if (right.isTop()) return top();
		
		if (operator instanceof AdditionOperator) {
			if (right.equals(left)) return EVEN;
			else 					return ODD;
		}
			...
			if (left.isEven() || right.isEven()) return EVEN;
			else								 return ODD;
