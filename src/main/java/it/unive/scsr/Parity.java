...
public class Parity implements BaseNonRelationalValueDomain<Parity> {
    ...
    public static final Parity EVEN = new Parity(0);

    public static final Parity ODD = new Parity(1);

    public static final Parity TOP = new Parity(-2);
    ...
    public Parity() { this.parity = 0;}

    public Parity(int parity) {  this.parity = parity; }
    ...
    public Parity evalNonNullConstant(Constant constant, ProgramPoint pp, SemanticOracle oracle) throws SemanticException {
        if (constant.getValue() instanceof Integer) {
            int tmp = (Integer) constant.getValue();
            if (tmp % 2 == 0) {
                return EVEN;
            } else {
                return ODD;
    ...
    public Parity evalBinaryExpression(BinaryOperator operator, Parity left, Parity right, ProgramPoint pp,
       SemanticOracle oracle) throws SemanticException {
        ...
        else if (operator instanceof AdditionOperator || operator instanceof SubtractionOperator) return right.equals(left) ? EVEN : ODD;
        else if (operator instanceof MultiplicationOperator) return left.isEven() || right.isEven() ? EVEN : ODD;
